...
public class UnfoldingService : IDisposable
{
	...
	{
		return this.UnfoldingData.FirstOrDefault(x => x.StepId == unfoldingStepId.Id) != null || this.CompletedUnfoldingStepIds.Contains(unfoldingStepId.Id);
	}
	...
	private void OnStateChanged(GameState state)
	{
		...
		{
			this.triggerService.MonitorTriggers(ufData.TriggerDatas, true).First(x => x).Subscribe(delegate(bool _)
			{
