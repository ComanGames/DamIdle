...
public class GildingService : IDisposable
{
	...
	public void PurchaseGilding(VentureModel ventureModel)
	{
		...
		AdCapStoreItem item = GameController.Instance.StoreService.CurrentCatalog.FirstOrDefault(x => x.Id == currentGildingItemID);
	...
	{
		this.AllVenturesBoosted.Value = GameController.Instance.game.VentureModels.All(x => x.IsBoosted.Value);
	}
	...
	private void GetPlatinumGildBoostConfig(double currentBoostValue)
	{
		...
		AdCapStoreItem adCapStoreItem = GameController.Instance.StoreService.CurrentCatalog.FirstOrDefault(x => x.Id == storeItemId);
		if (adCapStoreItem == null)
		{
			GameController.Instance.StoreService.CurrentCatalog.ObserveAdd().First(x => x.Value.Id == storeItemId).Subscribe(delegate(CollectionAddEvent<AdCapStoreItem> _)
			{
