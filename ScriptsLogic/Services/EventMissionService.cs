...
public class EventMissionService : IEventMissionService, IDisposable
{
	...
	public void Init(IGameController gameController, IDateTimeService dateTimeService, TimerService timerService, IAngelInvestorService angelInvestorService, IStoreService storeService, AdWatchService adService, UpgradeService upgradeService, IEventService eventService, IAnalyticService analyticService, ProfitBoostAdService profitBoostAdService)
	{
		...
		AdCapStoreItem adCapStoreItem = this.storeService.CurrentCatalog.FirstOrDefault(x => x.Id == "event_mission_refresh");
		if (adCapStoreItem == null)
		{
			this.storeService.CurrentCatalog.ObserveAdd().First(x => x.Value.Id == "event_mission_refresh").Subscribe(delegate(CollectionAddEvent<AdCapStoreItem> evt)
			{
	...
	private void ReplaceMission(UserEventMission mission)
	{
		...
		bool canBeGolden = this.currentMissions.Count(x => x.IsTimed) < this.currentStateConstants.MaxSuperTasks;
	...
	private IObservable<double> MonitorEventMission(UserEventMission mission)
	{
		...
		if (this.missionMonitorsByType.TryGetValue(mission.Type, out eventMissionMonitor))
		{
			return from v in eventMissionMonitor.MonitorEventMission(mission).CombineLatest(mission.State, (x, s) => new Tuple<double, EventMissionState>(x, s)).StartWith(new Tuple<double, EventMissionState>(mission.CurrentCount.Value, mission.State.Value)).TakeUntil((Tuple<double, EventMissionState> v) => v.Item1 >= mission.TargetAmount || v.Item2 == EventMissionState.EXPIRED)
			select v.Item1;
		}
	...
	private List<UserEventMission> GeneratePlanetMissions(bool isFirstGeneration, List<EventMissionType> pTypesToFilterOut = null, List<string> pVenturesToFilterOut = null)
	{
		...
		while (i < 4)
		{
			UserEventMission userEventMission = this.GeneratePlanetMission(list3, list2, list.Count(x => x.IsTimed) < this.currentStateConstants.MaxSuperTasks);
	...
	private UserEventMission GeneratePlanetMission(List<string> venturesToFilterOut, List<EventMissionType> missionTypesToFilterOut, bool canBeGolden)
	{
		...
		if (num3 >= 50.0 && num3 >= num4)
		{
			...
			{
				int num5 = this.upgradeService.Upgrades.Count(x => x.currency == Upgrade.Currency.AngelInvestors);
				int num6 = this.upgradeService.Upgrades.Count(x => x.currency == Upgrade.Currency.AngelInvestors && x.IsPurchased.Value);
		...
		if (!missionTypesToFilterOut.Contains(EventMissionType.CASH_UPGRADES_HAVE) && num2 <= (double)this.currentStateConstants.ContentCompleteExponent)
		{
			...
			int num8 = this.upgradeService.Upgrades.Count(x => x.currency == Upgrade.Currency.InGameCash);
			double num9 = allAffordableCashUpgrades.Sum(x => x.cost);
		...
		if (flag)
		{
			EventModel eventModel = this.eventService.ActiveEvents.FirstOrDefault(x => x.Id == this.currentState.planetName);
	...
	{
		EventMissionTaskTypeWeights eventMissionTaskTypeWeights = this.taskWeights.FirstOrDefault(x => x.Type == type);
	...
	private double CalculateInitialValue(EventMissionType type, string venture)
	{
		...
		{
			return this.currentState.VentureModels.FirstOrDefault(x => x.Id == venture).TotalOwned.Value;
		}
	...
	private double CalculateCashUpgradesHave(float difficulty_coef)
	{
		...
		List<Upgrade> allAffordableCashUpgrades = this.GetAllAffordableCashUpgrades(difficulty_coef);
		allAffordableCashUpgrades.Sum(x => x.cost);
		int num = allAffordableCashUpgrades.Count;
		num = this.upgradeService.Upgrades.Count(x => x.currency == Upgrade.Currency.InGameCash && x.IsPurchased.Value) + num;
		return (double)num;
	}
	...
	private double CalculateAngelUpgradeHave(float difficulty_coef)
	{
		...
		this.upgradeService.Upgrades.Count(x => x.currency == Upgrade.Currency.AngelInvestors && x.IsPurchased.Value);
	...
	private double CalculateMaxCanAffordVenture(float difficulty_coef, string venture)
	{
		...
		return this.currentState.VentureModels.First(x => x.Id == venture).CalculateMaxCanAfford(cashAmount);
	}
	...
	private double CalculateMaxVentureHave(float difficulty_coef, string venture)
	{
		...
		double value = this.currentState.VentureModels.First(x => x.Id == venture).TotalOwned.Value;
		if (num <= 0.0)
		{
			Debug.LogError(string.Concat(new object[]
			{
	...
	{
		VentureModel ventureModel = this.gameController.game.VentureModels.First(x => x.Id == venture);
		double value = this.angelInvestorService.AngelsOnHand.Value;
	...
	private void Update(TimeSpan deltaSpan)
	{
		...
		using (Dictionary<int, ReactiveProperty<double>>.Enumerator enumerator = this.MissionTimersById.GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				...
				if (this.currentMissions.FirstOrDefault(x => x.ID == pair.Key).State.Value == EventMissionState.ACTIVE)
				{
