...
public class TimeWarpService : IDisposable
{
	...
	{
		AdCapStoreItem adCapStoreItem = GameController.Instance.StoreService.CurrentCatalog.FirstOrDefault(x => x.Id == tweStoreItemId);
		if (adCapStoreItem == null)
		{
			this.storeItemDisposable = this.storeService.CurrentCatalog.ObserveAdd().First(x => x.Value.Id == tweStoreItemId).Subscribe(delegate(CollectionAddEvent<AdCapStoreItem> x)
			{
	...
	public void UseTimeWarpExpress()
	{
		...
		{
			if (GameController.Instance.game.VentureModels.Any(v => v.ProfitSurgeAmount((double)(hours * 3600), false) > 0.0))
			{
	...
	public void ApplyTimeWarp(double seconds, bool isExpress = false)
	{
		...
		double num = GameController.Instance.game.VentureModels.Sum(v => v.ProfitSurgeAmount(newSeconds, false));
	...
	private void MonitorUnlockCondition()
	{
		if (FeatureConfig.IsFlagSet("ShowTimeWarpUnfolding"))
		{
			...
			{
				VentureModel ventureModel = GameController.Instance.game.VentureModels.FirstOrDefault(model => model.Id.ToLower() == "hockey");
