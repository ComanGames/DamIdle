...
public class GrantRewardService : IGrantRewardService, IDisposable
{
	...
	{
		int index;
		VentureModel ventureModel = int.TryParse(rewardData.Id, out index) ? this.gameController.game.VentureModels[index] : this.gameController.game.VentureModels.FirstOrDefault(x => x.Id == rewardData.Id);
		...
		if (itemById != null)
		{
			...
			if (product <= Product.Unlock)
			{
				...
				default:
					...
					if (product == Product.Unlock)
					{
						Unlock unlock = this.gameController.UnlockService.Unlocks.Find(a => a.name == rewardData.Id);
