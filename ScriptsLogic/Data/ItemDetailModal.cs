...
public class ItemDetailModal : AnimatedModal, IBeginDragHandler, IEventSystemHandler, IEndDragHandler
{
	...
	protected override void Awake()
	{
		...
		this.nextItemButton.OnClickAsObservable().Subscribe(delegate(CanvasScaler.Unit __)
		{
			this.GoToNextItem();
		}).AddTo(base.gameObject);
		this.previousItemButton.OnClickAsObservable().Subscribe(delegate(CanvasScaler.Unit __)
		{
			this.GoToPreviousItem();
		}).AddTo(base.gameObject);
		this.closeButton.OnClickAsObservable().Subscribe(new Action<CanvasScaler.Unit>(this.CloseModal)).AddTo(base.gameObject);
	...
	public void ShowItem(AdCapStoreItem storeItem)
	{
		if (storeItem != null)
		{
			...
			foreach (KeyValuePair<string, GameObject> keyValuePair in this.purchaseButtons)
			{
				if (keyValuePair.Key == storeItem.Cost.Currency.ToString())
				{
					...
					IObservable<CanvasScaler.Unit> source = componentInChildren.OnClickAsObservable();
					Action<CanvasScaler.Unit> onNext;
					if ((onNext = <>9__2) == null)
					{
						onNext = (<>9__2 = delegate(CanvasScaler.Unit _)
						{
		...
		{
			this.CloseModal(CanvasScaler.Unit.Default);
		}).AddTo(base.gameObject);
	}
