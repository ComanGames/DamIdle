...
public class GameState
{
	...
	public GameState()
	{
		...
		this.TotalMultiplier = this.PurchasedMultiplier.CombineLatest(this.MultiplierBonus, this.SubscriptionMultiplier, (purchaseMult, multiplier, sub) => purchaseMult + multiplier + sub).Select(delegate(float v)
		{
			if (v != 0f)
			{
				return v;
	...
	public double TotalCashPerSecond()
	{
		...
		select venture).Sum(venture => venture.CashPerSec.Value);
	}
	...
	public void Reset(bool isAdWatch = false)
	{
		...
		using (List<Venture>.Enumerator enumerator = this.ventures.GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				...
				this.VentureModels.FirstOrDefault(v => v.Id == newVenture.id).Reset(newVenture.baseAmount, newVenture.costPer, newVenture.profitPer, (float)newVenture.cooldownTime);
			}
		}
	...
	public void InitVentureModels(IObservable<float> updateStream)
	{
		...
		using (List<Venture>.Enumerator enumerator = this.ventures.GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				...
				UnfoldingData unfoldingData2 = unfoldingData.FirstOrDefault(x => x.Target == venture.id && x.Type == UnfoldingData.EUnfoldingType.ShowVenture);
				UnfoldingData unfoldingData3 = unfoldingData.FirstOrDefault(x => x.Target == venture.id && x.Type == UnfoldingData.EUnfoldingType.UnlockVenture);
	...
	public void LoadGameSaveData(GameStateSaveData data)
	{
		...
		using (List<UnlockSaveData>.Enumerator enumerator = data.permanentUnlocks.GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				...
				Unlock unlock = this.Unlocks.Find(u => u.name == permanentUnlock.id);
				if (unlock != null)
				{
		...
		using (IEnumerator<VentureModel> enumerator2 = this.VentureModels.GetEnumerator())
		{
			while (enumerator2.MoveNext())
			{
				...
				VentureSaveData ventureSaveData = data.ventures.Find(v => v.id == venture.Id);
		...
		using (List<Upgrade>.Enumerator enumerator6 = this.Upgrades.GetEnumerator())
		{
			while (enumerator6.MoveNext())
			{
				...
				upgrade.LoadSaveData(data.upgrades.Find(u => u.id == upgrade.id));
		...
		using (List<Upgrade>.Enumerator enumerator6 = this.Managers.GetEnumerator())
		{
			while (enumerator6.MoveNext())
			{
				...
				manager.LoadSaveData(data.managers.Find(m => m.id == manager.id));
	...
	public void SetMultiplierBonus(GameState.MultiplierBonusType key, float value)
	{
		...
		this.MultiplierBonus.Value = this._MultiplierBonusMap.Sum(i => i.Value);
	}
