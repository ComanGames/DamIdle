...
public class RewardView : MonoBehaviour
{
	...
	public void WireData(IconService iconService, IInventoryService inventory, RewardData reward)
	{
		...
		case ERewardType.InvestmentQty:
		{
			...
			int index;
			VentureModel investment = int.TryParse(CS$<>8__locals1.reward.Id, out index) ? ventureModels[index] : ventureModels.FirstOrDefault(x => x.Id == CS$<>8__locals1.reward.Id);
			if (investment != null)
			{
