...
public class EventNavController : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		(from x in this.eventService.EventUnlocked.CombineLatest(MessageBroker.Default.Receive<WelcomeBackSequenceCompleted>(), (x, y) => x)
		where x
	...
	private void CheckPromo(bool isUnlocked)
	{
		...
		{
			EventModel eventModel = this.eventService.ActiveEvents.FirstOrDefault(x => x.ShouldShowEventPromo.Value);
			if (eventModel != null)
			{
