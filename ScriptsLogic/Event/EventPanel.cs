...
public class EventPanel : PanelBaseClass
{
	...
	private void SetupEventPanel(EventModel eventModel, Transform parent)
	{
		...
		ReadOnlyReactiveProperty<double> unlocksUnlocked = observable.CombineLatest(observable2, (g, m) => g + m).CombineLatest(observable3, (t, b) => t + b).CombineLatest(observable4, (t2, i) => t2 + i).CombineLatest(observable5, (t3, w) => t3 + w).CombineLatest(observable6, (t4, e) => t4 + e).CombineLatest(observable7, (t5, e) => t5 + e).ToReadOnlyReactiveProperty<double>();
