...
public class PlanetMissionPanel : PanelBaseClass
{
	...
	public void WireData(PlanetData planetData)
	{
		...
		this.storeItem = GameController.Instance.StoreService.CurrentCatalog.FirstOrDefault(x => x.Id == this.planetData.PurchaseId);
		...
		GameController.Instance.StoreService.CurrentCatalog.ObserveAdd().First(x => x.Value.Id == this.planetData.PurchaseId).Subscribe(new Action<CollectionAddEvent<AdCapStoreItem>>(this.OnPlanetUnlockItemAdded)).AddTo(base.gameObject);
	}
