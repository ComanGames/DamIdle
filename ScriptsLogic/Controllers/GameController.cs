...
public sealed class GameController : IGameController, IDisposable
{
    ...
    private IEnumerator WaitThenLoad(string planetName)
    {
        ...
        this.HhAssetBundleManager.GetBundleAsync("gamestate-" + this.planetName.ToLower()).Subscribe(delegate (IAssetBundle gameStateBundle)
        {
            ...
            Func<GameState_Serialized, bool> predicate =x => x.planetName == planetName;
            ...
            if (GameState.IsEvent(planetName))
            {
                ...
                Func<EventModel, bool> predicate2 = x => x.Id == planetName;
            ...
            Action<IAssetBundle> onNext = delegate (IAssetBundle planetBundle)
            {
                ...
                Func<PlanetData, bool> predicate3 = (x => x.PlanetName == planetTheme);
    ...
    {
        if (this.PlanetDataList.All(p => p.PlanetName != pPlanetName))
        {
    ...
    private void SetupStreams()
    {
        ...
        }, delegate (Action<VentureModel> h)
        {
         ...
         where this._State.Value.VentureModels.All(v => v.IsBoosted.Value)
        ...
        {
            VentureModel ventureModel = this._State.Value.VentureModels.FirstOrDefault(v => v.Id == "bank");
            if (ventureModel != null)
            {
                ventureModel.TotalOwned.First(n => n >= 1.0).Subscribe(delegate (double n)
                {
        ...
        {
            int num = this.UnlockService.Unlocks.Count(u => u.Earned.Value);
    ...
    {
        double num = this.game.VentureModels.Sum(venture => venture.ProfitSurgeAmount(elapsed, true));
        if (double.IsInfinity(num))
        {
            num = GameState.MAX_CASH_DOUBLE;
        }
