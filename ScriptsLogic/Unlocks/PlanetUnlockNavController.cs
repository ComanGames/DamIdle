...
public class PlanetUnlockNavController : MonoBehaviour
{
	...
	private void Awake()
	{
		GameController.Instance.State.First(x => x != null).Subscribe(new Action<GameState>(this.OnInitilized)).AddTo(base.gameObject);
	}
	...
	private void WireUpListner(PlanetData data)
	{
		if (!GameController.Instance.GlobalPlayerData.GetBool(data.PlanetName + "hasSeenUnlockModal"))
		{
			...
			{
				AdCapStoreItem adCapStoreItem = GameController.Instance.StoreService.CurrentCatalog.FirstOrDefault(x => x.Id == data.PurchaseId);
				if (adCapStoreItem != null)
				{
					this.OnPlanetUnlockItemAdded(adCapStoreItem, data);
					return;
				}
				(from x in GameController.Instance.StoreService.CurrentCatalog.ObserveAdd().First(x => x.Value.Id == data.PurchaseId)
	...
	private void OnPlanetUnlockItemAdded(AdCapStoreItem storeItem, PlanetData data)
	{
		...
		case Currency.MegaBuck:
			GameController.Instance.GlobalPlayerData.GetObservable("MegaBucksBalance", 0.0).First(x => x >= storeItem.Cost.Price).Subscribe(delegate(double _)
			{
				this.OnCanAffordPlanet(data);
			}).AddTo(base.gameObject);
			return;
		case Currency.InGameCash:
			GameController.Instance.game.CashOnHand.First(x => x >= storeItem.Cost.Price).Subscribe(delegate(double _)
			{
				this.OnCanAffordPlanet(data);
			}).AddTo(base.gameObject);
			break;
