...
public class TutorialController : MonoBehaviour
{
	...
	private IEnumerator WaitForTarget(string target)
	{
		...
		{
			hasTarget = false;
			this.tutorialService.TargetMap.ObserveAdd().First(x => x.Key == target).Subscribe(delegate(DictionaryAddEvent<string, List<GameObject>> x)
			{
				hasTarget = true;
	...
	private IEnumerator OpenPortraitMenu()
	{
		...
		this.navService.CurrentLocation.First(x => x == "Menu").Subscribe(delegate(string _)
		{
			openedMenu = true;
