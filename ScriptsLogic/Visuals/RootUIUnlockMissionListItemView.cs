...
public class RootUIUnlockMissionListItemView : MonoBehaviour
{
	...
	public void Start()
	{
		...
		GameController.Instance.IsInitialized.First(x => x).Subscribe(new Action<bool>(this.OnGameControllerInitiliazed)).AddTo(base.gameObject);
	}
	...
	public void WireData(Unlock u)
	{
		...
		if (this.currentUnlock is SingleVentureUnlock)
		{
			...
			GameController.Instance.game.VentureModels.FirstOrDefault(x => x.Name == single.ventureName).TotalOwned.Subscribe(delegate(double owned)
			{
		...
		else
		{
			...
			select new Tuple<double, double>(x.Sum(y => Math.Min(y, (double)this.currentUnlock.amountToEarn)), Math.Min(x.Min(), (double)this.currentUnlock.amountToEarn))).Subscribe(delegate(Tuple<double, double> pair)
			{
	...
	private void ShowReward(Unlock unlock)
	{
		...
		if (unlock.Reward is UnlockRewardBadge)
		{
			...
			select v).Subscribe(delegate(bool x)
			{
				...
				Action<Image> action = (img) => img.sprite = GameController.Instance.IconService.GetBadgeIcon(item.IconName);
				list.ForEach(action);
			}).AddTo(base.gameObject);
			return;
		}
