...
public class ProfitBooster
{
	...
	public ProfitBooster(GameState game, ProfitBoostSetup setup, IObservable<Unit> onDeployClick, IObservable<Unit> onFightBackClick, ProfitBoosterType profitBoosterType)
	{
		...
		this.TotalBoost = this.boostEffect.CombineLatest(this.ProfitBoosterBoost, (bbe, pbb) => (float)bbe + pbb).CombineLatest(this.boostAffectorEffect, (tb, bae) => tb + (float)bae).CombineLatest(this.itemEffectBonus, (tb, ieb) => tb * (1f + ieb)).ToReadOnlyReactiveProperty<float>();
	...
	private void Deploy()
	{
		...
		VentureModel ventureModel2 = this.game.VentureModels.FirstOrDefault(v => v.Id == this.affectedVentureId);
