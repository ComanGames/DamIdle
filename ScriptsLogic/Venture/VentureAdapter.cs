...
public static class VentureAdapter
{
	...
	public static void InitView(VentureModel model, VentureView view, List<Sprite> sprites, VentureViewColorData colorData, bool isEventPlanet, ClampedReactiveDouble cashOnHand, Action<VentureModel> buyVentureAction, Action<double> onFinished, Action<VentureModel> boostVentureAction)
	{
		...
		model.TotalOwned.CombineLatest(model.AchievementTarget.StartWith(0), (o, t) => o).Subscribe(delegate(double o)
		{
		...
		model.TotalOwned.CombineLatest(model.AchievementTarget, (totalOwned, achievementTarget) => VentureAdapter.UpdateAchievmentProgress((float)achievementTarget, (float)model.LastAchievementTarget.Value, totalOwned)).Subscribe(delegate(float x)
		{
		...
		model.CostForNext.CombineLatest(GameController.Instance.game.CashOnHand, (cost, cash) => cost).CombineLatest(model.CanAfford, (cost, bn) => new Tuple<double, double>(cost, bn)).Subscribe(delegate(Tuple<double, double> tuple)
		{
