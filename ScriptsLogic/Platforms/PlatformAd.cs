...
namespace Platforms
{
	...
	public class PlatformAd : IPlatformAd, IDisposable
	{
		...
		public IObservable<ShowResult> ShowAd(AdType type, string placementId = "")
		{
			...
			IPlatformAdExtension platformAdExtension = source.FirstOrDefault(x => x.AdReady[type].Value);
		...
		private void UpdateAdReady(AdType type)
		{
			...
			this.AdReadyMap[type].Value = this.platformAdMap[type].Any(x => x.AdReady[type].Value);
		}
