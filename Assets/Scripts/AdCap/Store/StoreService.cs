	...
	public class StoreService : IStoreService, IDisposable
	{
		...
		public bool AttemptPurchase(AdCapStoreItem item, Currency currency)
		{
			...
			if (itemById != null)
			{
				...
				else if (this.upgradeService.Managers.Count((Upgrade m) => m.IsPurchased.Value) == 0)
				{
		...
		{
			KeyValuePair<string, AdCapStoreItem> keyValuePair = this.StoreItemMap.FirstOrDefault((KeyValuePair<string, AdCapStoreItem> x) => x.Value.PlatformId == evt.Item.ItemId);
		...
		{
			ItemPurchaseData itemPurchaseData = this.PurchaseData.FirstOrDefault((ItemPurchaseData x) => x.ItemId == storeItem.Id);
		...
		private void WireStoreData()
		{
			...
			select x).Sum((ItemPurchaseData x) => x.TotalPurchaseCount);
			this.StoreItemMap = source.ToDictionary((AdCapStoreItem item) => item.Id);
		...
		private void ProcessCatalog(List<CatalogStoreItem> catalog)
		{
			using (Dictionary<string, AdCapStoreItem>.Enumerator enumerator = this.StoreItemMap.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					...
					CatalogStoreItem catalogStoreItem = catalog.FirstOrDefault((CatalogStoreItem x) => x.ItemId == storeItem.Value.PlatformId);
		...
		private void WireStoreItem(AdCapStoreItem item)
		{
			...
			ItemPurchaseData itemPurchaseData = this.PurchaseData.FirstOrDefault((ItemPurchaseData x) => x.ItemId == item.Id);
		...
		private void WireExistingStoreItem(AdCapStoreItem storeItem)
		{
			...
			if (itemPurchaseData != null && !itemPurchaseData.IsLastChance && (storeItem.PurchaseCount == 0 || storeItem.PurchaseCount > itemPurchaseData.CurrentPurchaseCount.Value))
			{
				...
				if (storeItem.ExpiryDate > utcNow)
				{
					...
					storeItem.TimeRemaining.First((double x) => x <= 0.0).Subscribe(delegate(double _)
					{
		...
		private void OnNewItemAvailable(AdCapStoreItem storeItem)
		{
			...
			this.PurchaseData.FirstOrDefault((ItemPurchaseData x) => x.ItemId == storeItem.Id).ExpiryDate = storeItem.ExpiryDate;
		...
		private void CreatePurchaseDataForActiveItems()
		{
			using (Dictionary<string, long>.Enumerator enumerator = this.activeItemMap.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					...
					ItemPurchaseData itemPurchaseData = this.PurchaseData.FirstOrDefault((ItemPurchaseData x) => x.ItemId == kvp.Key);
					if (itemPurchaseData == null)
					{
