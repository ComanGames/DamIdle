...
public class MegaTicketsPanel : PanelBaseClass
{
	...
	private void Start()
	{
		...
		GameController.Instance.IsInitialized.First((bool x) => x).Subscribe(delegate(bool state)
		{
			this.Init();
	...
	{
		AdCapStoreItem storeItem = GameController.Instance.StoreService.CurrentCatalog.FirstOrDefault((AdCapStoreItem x) => x.Id == itemId);
		if (storeItem != null)
		{
			this.WirePurchaseButton(btn, price_label, itemId, storeItem);
			return;
		}
		(from x in GameController.Instance.StoreService.CurrentCatalog.ObserveAdd().First((CollectionAddEvent<AdCapStoreItem> x) => x.Value.Id == itemId)
