...
public class FTUE_Manager : MonoBehaviour
{
	...
	private void OnStateChanged(GameState state)
	{
		...
		if (state.planetName == "Mars")
		{
			...
			{
				GameController.Instance.NavigationService.CurrentLocation.First((string x) => x == "Root").Subscribe(delegate(string __)
				{
					FTUE_Manager.ShowFTUE("ProfitMartians", null);
				}).AddTo(this.gameStateDisposable);
				return;
			}
			MessageBroker.Default.Receive<FTUEFinished>().First((FTUEFinished x) => x.FTUEName == "FirstEarnedSuitPiece").Subscribe(delegate(FTUEFinished _)
			{
				GameController.Instance.NavigationService.CurrentLocation.First((string x) => x == "Root").Subscribe(delegate(string __)
				{
					FTUE_Manager.ShowFTUE("ProfitMartians", null);
	...
	private void ShowSignificantAngelResetReminderFTUE()
	{
		...
		FTUE_Manager.FTUE_Element ftue_Element = this.ftueElements.FirstOrDefault((FTUE_Manager.FTUE_Element x) => x.ftueName == "AngelResetThreshold");
	...
	{
		FTUE_Manager.FTUE_Element ftue_Element = this.ftueElements.Find((FTUE_Manager.FTUE_Element e) => e.ftueName == ftueName);
	...
	private void UpdateFTUEContent()
	{
		...
		if (OrientationController.Instance.CurrentOrientation.IsPortrait)
		{
			if (ftue_Content.isMenu && !this.hasClickedMenu)
			{
				...
				GameController.Instance.NavigationService.CurrentLocation.First((string x) => x == "Menu").Subscribe(delegate(string _)
				{
	...
	private void OnVenturePurchase(VentureModel venture)
	{
		...
		{
			venture.TotalOwned.First((double x) => x >= this.AdWatchTrigger.Qty).Subscribe(delegate(double _)
			{
	...
	private IEnumerator FirstFTUE()
	{
		...
		VentureModel lemons = GameController.Instance.game.VentureModels.FirstOrDefault((VentureModel v) => v.Id == "lemon");
		if (lemons == null)
		{
			yield break;
		}
		...
		this.NextFTUE();
		VentureModel news = GameController.Instance.game.VentureModels.FirstOrDefault((VentureModel v) => v.Id == "news");
	...
	private IEnumerator OfferwallFTUE()
	{
		...
		VentureModel oil = GameController.Instance.game.VentureModels.FirstOrDefault((VentureModel v) => v.Id == "oil");
	...
	private IEnumerator MonitorManagersFTUE()
	{
		...
		for (;;)
		{
			if (GameController.Instance.UpgradeService.Managers.FindAll((Upgrade m) => m.currency == Upgrade.Currency.InGameCash && !m.IsPurchased.Value && m.cost <= GameController.Instance.game.CashOnHand.Value).Count != 0)
			{
				break;
			}
			yield return new WaitForSeconds(1f);
		...
		for (;;)
		{
			if (GameController.Instance.game.VentureModels.Count((VentureModel v) => v.IsManaged.Value) > 5)
			{
				break;
			}
			yield return new WaitForSeconds(1f);
