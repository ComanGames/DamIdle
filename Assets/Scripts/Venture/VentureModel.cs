...
public class VentureModel
{
	...
	{
		if (this.visableTriggers.TrueForAll((TriggerData x) => GameController.Instance.TriggerService.CheckTrigger(x)))
		{
			this.UnlockState.Value = VentureModel.EUnlockState.Locked;
			this.CheckUnlockTrigger();
			return;
		}
		GameController.Instance.TriggerService.MonitorTriggers(this.visableTriggers, true).First((bool x) => x).Subscribe(delegate(bool _)
		{
			this.UnlockState.Value = VentureModel.EUnlockState.Locked;
			this.CheckUnlockTrigger();
	...
	{
		if (this.unlockTriggers.All((TriggerData x) => GameController.Instance.TriggerService.CheckTrigger(x)))
		{
			this.UnlockState.Value = VentureModel.EUnlockState.Unlocked;
			this.CheckForPurchased();
			return;
		}
		GameController.Instance.TriggerService.MonitorTriggers(this.unlockTriggers, true).First((bool x) => x).Subscribe(delegate(bool _)
		{
			this.UnlockState.Value = VentureModel.EUnlockState.Unlocked;
			this.CheckForPurchased();
		}).AddTo(this.unlockStateDisposables);
	...
	{
		this.TotalOwned.First((double x) => x >= 1.0).Subscribe(delegate(double _)
		{
	...
	{
		VentureTimedSpeedBoost ventureTimedSpeedBoost = this.TimedSpeedBoosts.Find((VentureTimedSpeedBoost b) => b.id == id);
