...
public sealed class SwagPanel : PanelBaseClass
{
	...
	private void GetSwagAchievementNumbers()
	{
		...
		{
			this.levelsPortrait[i].achievementActivator = unlocks.FindIndex((Unlock a) => a.name == this.levelsPortrait[i].achievementName);
			this.levelsLandscape[i].achievementActivator = unlocks.FindIndex((Unlock a) => a.name == this.levelsLandscape[i].achievementName);
			j = i;
		}
	}
	...
	private void ShowTimeCarIfUnlocked()
	{
		...
		SwagLevel swagLevel = this.levelsPortrait.FirstOrDefault((SwagLevel l) => l.name == "Time Car");
		if (swagLevel != null)
		{
			swagLevel.images[0].enabled = enabled;
		}
		SwagLevel swagLevel2 = this.levelsLandscape.FirstOrDefault((SwagLevel l) => l.name == "Time Car");
		if (swagLevel2 != null)
		{
	...
	{
		return achievement != null && this.levelsPortrait.Any((SwagLevel level) => level.achievementName == achievement.name);
	}
