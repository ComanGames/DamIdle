...
public class EventMissionsModal : AnimatedModal
{
	...
	public override void Init(NavigationService navService)
	{
		...
		EventModel eventModel = this.eventService.ActiveEvents.FirstOrDefault((EventModel x) => x.Id == GameController.Instance.game.planetName);
		if (eventModel != null)
		{
	...
	{
		UserEventMission firstCompleteMission = this.eventMissionService.currentMissions.FirstOrDefault((UserEventMission x) => x.Type == EventMissionType.FIRST_AUTO_COMPLETE);
	...
	{
		if (this.eventMissionService.currentMissions.Any((UserEventMission x) => x.State.Value == EventMissionState.COMPLETE))
		{
	...
	private void ReturnListItemToPool(BaseMissionListItem listItem)
	{
		...
		UserEventMission key = this.listElementsByMission.First((KeyValuePair<UserEventMission, BaseMissionListItem> x) => x.Value == listItem).Key;
