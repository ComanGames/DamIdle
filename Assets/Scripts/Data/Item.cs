...
public class Item : IDisposable
{
	...
	private string GetInstantProfitString(string preparedDescription, double bonusData)
	{
		...
		double num = GameController.Instance.game.VentureModels.Sum((VentureModel v) => v.ProfitSurgeAmount(bonusTimeTotal, false));
		if (num == 0.0)
		{
	...
	private string GetVentureTargetName()
	{
		...
		if (num != 1)
		{
			if (num == 2)
			{
				VentureModel ventureModel = GameController.Instance.game.VentureModels.FirstOrDefault((VentureModel x) => x.Id == splitString[1]);
