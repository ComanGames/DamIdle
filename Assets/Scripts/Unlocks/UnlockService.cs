...
public class UnlockService : IDisposable
{
	...
	public void Init()
	{
		...
		GameController.Instance.IsInitialized.First((bool x) => x).Subscribe(new Action<bool>(this.OnGameControllerInitiliazed)).AddTo(this.disposables);
	}
	...
	private void OnStateChanged(GameState state)
	{
		...
		foreach (VentureModel ventureModel2 in this.state.VentureModels)
		{
			...
			if (!this.AutoClaimFirstTimeUnlocks)
			{
				...
				select x.Count((bool y) => y)).CombineLatest(from x in sources2.CombineLatest<bool>()
				select x.Count((bool y) => y), (int claimedCount, int earnedCount) => earnedCount - claimedCount).Subscribe(delegate(int unlocksToClaim)
				{
	...
	public void TriggerUnlockEvent(Unlock unlock)
	{
		...
		{
			SingleVentureUnlock singleVentureUnlock = (SingleVentureUnlock)this.Unlocks.FirstOrDefault((Unlock a) => a is SingleVentureUnlock && ((SingleVentureUnlock)a).ventureName == ((SingleVentureUnlock)unlock).ventureName && !a.Earned.Value);
	...
	public void ClaimUnlock(Unlock unlock)
	{
		...
		if (this.Unlocks.All((Unlock u) => u.Claimed.Value))
		{
	...
	public void SetAllNextUnlocks()
	{
		using (IEnumerator<VentureModel> enumerator = this.state.VentureModels.GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				...
				SingleVentureUnlock singleVentureUnlock = (SingleVentureUnlock)this.Unlocks.FirstOrDefault((Unlock a) => a is SingleVentureUnlock && ((SingleVentureUnlock)a).ventureName == model.Name && !a.Earned.Value);
				if (singleVentureUnlock != null)
				{
					SingleVentureUnlock singleVentureUnlock2 = (SingleVentureUnlock)this.Unlocks.LastOrDefault((Unlock a) => a is SingleVentureUnlock && ((SingleVentureUnlock)a).ventureName == model.Name && a.Earned.Value);
	...
	private void UpdateNextUnlocks()
	{
		...
		using (IEnumerator<VentureModel> enumerator = this.state.VentureModels.GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				...
				Unlock unlock3 = source.FirstOrDefault((SingleVentureUnlock unlock) => unlock != null && unlock.ventureName == venture.Name);
				if (unlock3 != null)
				{
		...
		Unlock unlock2 = source2.FirstOrDefault((EveryVentureUnlock unlock) => unlock != null);
		if (unlock2 != null)
		{
