...
public class InventoryService : IInventoryService, IDisposable
{
	...
	public void LoadInventoryData(List<Item> data)
	{
		...
		using (List<string>.Enumerator enumerator2 = list.GetEnumerator())
		{
			while (enumerator2.MoveNext())
			{
				...
				if (!this.allItems.Any((KeyValuePair<string, Item> x) => x.Key == itemId))
				{
	...
	{
		return this.equipItemSlots[ItemType.Badge].All((Item item) => item != null);
	}
	...
	{
		return (from item in this.equipItemSlots.SelectMany((KeyValuePair<ItemType, Item[]> x) => x.Value)
		where item != null
