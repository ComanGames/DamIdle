...
public class KongAnalyticService : IAnalyticService, IDisposable
{
	...
	private void InitializeGameEventsAnalyticsHooks()
	{
		...
		{
			int count = this.gameController.UnlockService.Unlocks.FindAll((Unlock a) => a.Earned.Value).Count;
			string planetName = this.gameController.game.planetName;
			Dictionary<string, object> evt = new Dictionary<string, object>
			{
				{
	...
	private void OnGameStateChanged(GameState state)
	{
		...
		state.planetPlayerData.GetObservable("MegaBucksCreated", 0.0).Pairwise((double x, double y) => y - x).Subscribe(delegate(double x)
		{
	...
	private void InitializePurchaseAnalyticsHooks()
	{
		...
		{
			AdCapStoreItem adCapStoreItem = this.gameController.StoreService.CurrentCatalog.FirstOrDefault((AdCapStoreItem x) => x.PlatformId == e.Item.ItemId);
	...
	{
		Dictionary<string, object> dictionary = new Dictionary<string, object>();
		AdCapStoreItem adCapStoreItem = GameController.Instance.StoreService.CurrentCatalog.FirstOrDefault((AdCapStoreItem x) => x.Id == productId);
