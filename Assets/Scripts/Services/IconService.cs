...
public class IconService
{
	...
	public Sprite GetGoldIcon(int amount)
	{
		...
		Icon icon = this.icons.FirstOrDefault((Icon x) => x.id == iconName);
		if (icon != null)
		{
			return icon.sprite;
		}
	...
	{
		if (this.icons.Exists((Icon i) => i.id == id))
		{
			return this.icons.Find((Icon i) => i.id == id).sprite;
		}
		return this.icons.Find((Icon i) => i.id == "default").sprite;
	}
	...
	public void LoadIconsFromScriptableObject(IconsScriptableObject iconScriptableObject)
	{
		using (List<Icon>.Enumerator enumerator = iconScriptableObject.Icons.GetEnumerator())
		{
			while (enumerator.MoveNext())
			{
				...
				if (!this.icons.Exists((Icon i) => i.id == icon.id))
				{
					this.icons.Add(icon);
				}
			}
