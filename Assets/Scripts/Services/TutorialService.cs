...
public class TutorialService : IDisposable
{
	...
	{
		return this.currentTutorialBlockList.Any((TutorialBlock x) => x.BlockId == blockId);
	}
	...
	private void OnStateChanged(GameState state)
	{
		...
		if (list.Count > 0)
		{
			...
			{
				this.triggerService.MonitorTriggers(block.ResetTriggers, false).First((bool x) => x).Subscribe(delegate(bool _)
				{
	...
	private void OnStepTriggered(TutorialStep step)
	{
		...
		{
			this.triggerService.MonitorTriggers(step.EndTriggers, false).First((bool x) => x).DelayFrame(1, FrameCountType.Update).Subscribe(delegate(bool __)
			{
				this.NextStep();
			}).AddTo(this.stepDisposables);
		}
	...
	private void WireTutorialBlock(TutorialBlock block)
	{
		...
		this.triggerService.MonitorTriggers(block.StartTriggers, true).First((bool x) => x).Subscribe(delegate(bool _)
		{
			this.OnTutorialBlockTriggered(block);
		}).AddTo(this.tutorialBlockDisposableMap[block.BlockId]);
		if (block.SkipTriggers.Count > 0)
		{
			this.triggerService.MonitorTriggers(block.SkipTriggers, true).First((bool x) => x).Subscribe(delegate(bool _)
			{
				this.OnTutorialBlockSkipped(block);
			}).AddTo(this.tutorialBlockDisposableMap[block.BlockId]);
		}
	...
	private void OnBlockFinished()
	{
		...
		TutorialBlock completedBlock = this.currentTutorialBlockList.FirstOrDefault((TutorialBlock x) => x.BlockId == this.CurrentStep.Value.BlockId);
		...
		{
			this.triggerService.MonitorTriggers(completedBlock.ResetTriggers, false).First((bool x) => x).Subscribe(delegate(bool _)
			{
	...
	public void SkipTutorialBlock(string id)
	{
		...
		{
			this.OnTutorialBlockSkipped(this.currentTutorialBlockList.FirstOrDefault((TutorialBlock x) => x.BlockId == id));
		}
	}
