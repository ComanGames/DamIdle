...
public class TriggerService : ITriggerService, IDisposable
{
	...
	{
		return (from s in this.state.ObserveEveryValueChanged((ReactiveProperty<GameState> x) => x.Value, FrameCountType.Update, false)
		where s != null
		select s).CombineLatest(this.angelService.RewardAngelsAtInterval, this.isLoadingPlanet, (GameState s, double pendingAngels, bool isLoading) => !isLoading && s.planetTheme.Value == triggerData.PlanetId && this.Compare<double>(triggerData.Operator, pendingAngels, threshold)).StartWith(false);
	}
	...
	private IObservable<bool> MonitorCashOnHand(TriggerData triggerData, double threshold)
	{
		...
		{
			return (from t in (from s in this.state.ObserveEveryValueChanged((ReactiveProperty<GameState> x) => x.Value, FrameCountType.Update, false)
			where s != null
			select s).CombineLatest(this.state.Value.CashOnHand, (GameState s, double cash) => new Tuple<string, double>(s.planetTheme.Value, cash))
	...
	private IObservable<bool> MonitorVentureQuantity(TriggerData triggerData, double threshold)
	{
		...
		if (!string.IsNullOrEmpty(triggerData.PlanetId))
		{
			...
			if (this.saveDataMap.ContainsKey(triggerData.PlanetId) && this.saveDataMap[triggerData.PlanetId] != null)
			{
				VentureSaveData ventureSaveData = this.saveDataMap[triggerData.PlanetId].ventures.FirstOrDefault((VentureSaveData x) => x.id == triggerData.Id);
	...
	private IObservable<bool> MonitorAngelClaimedQuantity(TriggerData triggerData, double threshold)
	{
		if (this.state.Value != null && this.state.Value.planetTheme.Value == triggerData.PlanetId)
		{
			return (from x in (from x in this.state.ObserveEveryValueChanged((ReactiveProperty<GameState> x) => x.Value, FrameCountType.Update, false)
			where x != null
			select x).CombineLatest(this.angelService.AngelsOnHand, (GameState d, double p) => new Tuple<GameState, double>(this.state.Value, this.angelService.AngelsOnHand.Value + this.angelService.AngelsSpent.Value)).Where((Tuple<GameState, double> x, int y) => x.Item1.planetTheme.Value == triggerData.PlanetId)
	...
	private IObservable<bool> MonitorAngelResetQuantity(TriggerData triggerData, double threshold)
	{
		if (this.state.Value != null && this.state.Value.planetTheme.Value == triggerData.PlanetId)
		{
			return (from x in (from x in this.state.ObserveEveryValueChanged((ReactiveProperty<GameState> x) => x.Value, FrameCountType.Update, false)
			where x != null
			select x).CombineLatest(this.angelService.AngelResetCount, (GameState d, int p) => new Tuple<GameState, double>(this.state.Value, (double)p)).Where((Tuple<GameState, double> x, int y) => x.Item1.planetTheme.Value == triggerData.PlanetId)
			select this.Compare<double>(triggerData.Operator, x.Item2, threshold)).StartWith(false);
		}
		if (!string.IsNullOrEmpty(triggerData.PlanetId))
		{
	...
	{
		return this.storeService.PurchaseData.Any((ItemPurchaseData x) => x.ItemId == id && x.TotalPurchaseCount > 0);
	}
