...
public class QuickBuyUpgradeView : MonoBehaviour
{
	...
	private void Start()
	{
		GameController.Instance.IsInitialized.First((bool init) => init).Subscribe(delegate(bool __)
		{
			GameController.Instance.UpgradeService.IsInitialized.First((bool init) => init).Subscribe(delegate(bool ___)
			{
	...
	private void OnUpgradePurchaseableChanged()
	{
		...
		{
			text2 = string.Format(format, NumberFormat.Convert(quickPurchaseableUpgrades.Sum((Upgrade up) => up.cost), 1000000000.0, true, 3));
		}
		text.text = text2;
	}
