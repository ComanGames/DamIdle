...
public class UpgradeService : IDisposable
{
	...
	{
		Upgrade value = this.Upgrades.FirstOrDefault((Upgrade up) => !up.IsPurchased.Value && up.currency == Upgrade.Currency.InGameCash);
	...
	private void OnCashOnHandChanged(double cashOnHand)
	{
		...
		{
			if (!this.Upgrades.Any((Upgrade cashUpgrade) => cashUpgrade.IsPurchaseable.Value && cashUpgrade.currency == Upgrade.Currency.InGameCash))
			{
				this.NewUpgradeAvailableMap[Upgrade.Currency.InGameCash].Value = false;
			}
		}
		...
		{
			if (!this.Managers.Any((Upgrade cashUpgrade) => cashUpgrade.IsPurchaseable.Value && cashUpgrade.currency == Upgrade.Currency.InGameCash))
			{
				this.NewManagerAvailableMap[Upgrade.Currency.InGameCash].Value = false;
			}
		}
	...
	private void OnAngelsOnHandChanged(double angelsOnHand)
	{
		...
		{
			if (!this.Upgrades.Any((Upgrade cashUpgrade) => cashUpgrade.IsPurchaseable.Value && cashUpgrade.currency == Upgrade.Currency.AngelInvestors))
			{
				this.NewUpgradeAvailableMap[Upgrade.Currency.AngelInvestors].Value = false;
			}
		}
		...
		{
			if (!this.Managers.Any((Upgrade cashUpgrade) => cashUpgrade.IsPurchaseable.Value && cashUpgrade.currency == Upgrade.Currency.AngelInvestors))
			{
				this.NewManagerAvailableMap[Upgrade.Currency.AngelInvestors].Value = false;
			}
		}
	...
	{
		int num = this.Upgrades.Count((Upgrade u) => u.IsPurchased.Value);
