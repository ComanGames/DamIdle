...
public class WelcomeBackAdMultiplierView : AnimatedModal
{
	...
	public void Start()
	{
		this.offlineCashMultiplier = GameController.Instance.StoreService.CurrentCatalog.FirstOrDefault((AdCapStoreItem x) => x.Id == "offline_cash_multiplier");
		if (this.offlineCashMultiplier == null)
		{
			this.btn_Purchase.gameObject.SetActive(false);
			GameController.Instance.StoreService.CurrentCatalog.ObserveAdd().First((CollectionAddEvent<AdCapStoreItem> x) => x.Value.Id == "offline_cash_multiplier").Subscribe(new Action<CollectionAddEvent<AdCapStoreItem>>(this.OnOfflineItemReceived)).AddTo(base.gameObject);
		}
		else
		{
